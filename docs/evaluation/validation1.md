# Validation Report 1

作成日: 2026-03-15

目的: `docs/spec/spec.md` の要求項目と、現在 GitHub Pages に公開されているコンテンツおよびリポジトリ内実装を照合し、「できたこと」と「できなかったこと」を整理する。

**参照**:
- 仕様: [docs/spec/spec.md](docs/spec/spec.md)
- 公開ページ (Pages): https://masayukiyoko-gcp.github.io/world_watch001/
- 実装/検証対象ファイル: [public/index.html](public/index.html#L1-L200), [src/js/clock.js](src/js/clock.js#L1), [tests/unit/clock.test.js](tests/unit/clock.test.js#L1-L20), [cypress/e2e/app_spec.cy.js](cypress/e2e/app_spec.cy.js#L1-L40), [scripts/screenshot.js](scripts/screenshot.js)

## できたこと

- 静的 Web アプリを `public/` に実装し、GitHub Pages で公開済み（URL: 上記）。
- 公開ページはローカルサーバと概ね同等に表示されるよう修正済（`public/index.html` の資産参照を相対パスに変更）。
- 基本的な時刻表示ロジックを `src/js/clock.js` に実装し、単体テスト（`tests/unit/clock.test.js`）を追加済み。現在のユニットテストは動作確認レベルで通過しています。
- E2E テストフレームワークとして Cypress を導入し、`cypress/e2e/app_spec.cy.js` にて主要要素（UTC / Tokyo の時刻表示）を検証するテストを用意。
- Playwright を導入し、公開ページのスクリーンショットを取得する `scripts/screenshot.js` をリポジトリに追加済み。
- リポジトリ管理: `node_modules/` と Pages の生成物（`pages_index.html` / `artifacts/` / `assets/screenshots/`）を `.gitignore` に追加し、生成物を除外する運用を確立。
- CI/ワークフローは PR にスクリーンショットを要求する方針を含めて追加済み（ワークフロー定義は `.github/workflows/` にあり、表示系の検証手順を導入済）。

## できていないこと（ギャップ）

- 仕様の主要機能の不実装:
  - アナログ時計表示（仕様はアナログ表示を要求）→ 現状はテキスト形式の HH:MM:SS 表示のみ。
  - 時計の数を動的に 2〜8 に変更する UI → 未実装（現在は固定で UTC と Tokyo を表示）。
  - 文字盤の色で昼夜を示す機能 → 未実装。
  - 都市名の部分一致による候補リストと選択機能 → 未実装。
  - 多言語対応（日本語・英語・タイ語・インドネシア語）の UI 切替 → 未実装（未整備）。

- テスト網羅性:
  - 単体テストの条件網羅（C0）・分岐網羅（C1）を要求しているが、現状の単体テストは限定的であり、C0/C1 を満たしていないモジュールが多数存在する。
  - 表示系のテストは基本的な E2E を用意しているが、スクリーンショット比較や視覚的回帰テストの自動化は限定的／部分的実装にとどまる。

- ドキュメントや運用の未整備点:
  - 多数の仕様（都市候補のデータソース、i18n の文字列管理方法など）に対する実装方針が未確定で、残作業が多い。

## 推奨アクション（優先度順）

1. コア機能の優先実装: `src/js` と `public/js` に対して、
   - 都市データの導入（簡易 JSON）と部分一致検索
   - 時計の動的追加/削除 UI
   - アナログ表示コンポーネント（既存ライブラリ or Canvas/SVG 実装）

2. テスト強化:
   - `src/js/clock.js` 等のロジックに対するユニットテストを拡充し、C0/C1 を満たすようにする（カバレッジ測定を CI に導入）。
   - 表示系についてはスクリーンショット取得 → 差分比較（例: Pixelmatch）を CI に組み込む。

3. i18n 設計: 言語切替のための `locales/` ディレクトリと運用手順を定義する。

4. ドキュメント: `docs/spec` の各要件に対し、現状ステータス（実装／部分実装／未実装）をテーブル化して管理する（次回の validation で追跡）。

---

この検証レポートはリポジトリ内の現状実装（2026-03-15）に基づき作成しました。詳細な差分（ファイルレベルの証跡やテスト出力）が必要であれば追って添付します。

## 要件チェックリスト

以下は `docs/spec/spec.md` の主要要件を抜粋し、現時点での達成状況を示します。

1. 静的サイトとして世界時計を構築し GitHub Pages で公開する — **達成**
   - 根拠: 公開 URL と `public/` のデプロイ確認（[public/index.html](public/index.html#L1-L200)）

2. アナログ時計で表示する — **未達成**
   - 備考: 現状はデジタル表示（HH:MM:SS）のみ。`public` 側の UI を拡張する必要あり。

3. 時計の数を動的に 2〜8 で変更可能にする — **未達成**

4. 文字盤の色で昼夜を示す機能 — **未達成**

5. 都市名の部分一致候補リストで選択可能にする — **未達成**

6. 表示言語（日本語・英語・タイ語・インドネシア語）を切替可能にする — **未達成**

7. UI レイアウト要件（世界地図背景、都市名、アナログ時計群、コントロール） — **部分達成**
   - 備考: 時計表示はあるが地図背景・コントロール・多言語ラベルなどは未実装

8. 単体テストの条件網羅(C0)・分岐網羅(C1)を満たすこと — **未達成**
   - 根拠: `tests/unit/clock.test.js` 等は存在するが網羅性は不足

9. 表示系の検証にスクリーンショットを必須とする（PRルール） — **部分達成**
   - 根拠: CI ワークフローでスクリーンショットの存在をチェック（`.github/workflows/ci.yml` の `enforce-screenshots`）
   - 備考: スクリーンショット取得スクリプト `scripts/screenshot.js` はあるが、CI のスクリーンショットアーティファクト等に恒常的なファイルが存在するわけではない

10. ローカルサーバでの動作確認手順を用意する — **達成**
   - 備考: `scripts/start_local_server.md` 等の手順によりローカル確認が可能

11. copilot による Issue 実行フローの運用自動化 — **達成**
   - 備考: Issue 自動作成・自動クローズ等のスクリプトとワークフローが存在

この一覧を基に、各要件に対する具体的なタスク（担当・見積もり・優先度）を追加していくことを推奨します。

## モジュール別カバレッジ（実測 / 状況）

以下はローカルで実行したカバレッジ測定の結果と注記です（測定日: 2026-03-15）。

- `src/js/clock.js` — C0 (ステートメント) = 100% / C1 (分岐) = 100%
   - 備考: 簡易ランナー `tests/run_clock_runner.js` を c8 で実行して測定。モジュールは分岐が無いため C0=C1=100%

- `src/modules/` — 該当ファイルなし → カバレッジ未測定（N/A）

- `src/utils/` — 該当ファイルなし → カバレッジ未測定（N/A）

- `src/config/` — 該当ファイルなし → カバレッジ未測定（N/A）

- フロントエンド実装 (`public/js/app.js`) — Node 環境での自動計測が難しく、現状は未測定
   - 備考: ブラウザ環境での計測が必要（Karma + Istanbul 等、あるいは E2E 実行中のカバレッジ収集が有用）

※補足: 今回はローカルで `npx c8 node tests/run_clock_runner.js` を実行し、`src/js/clock.js` のカバレッジを取得しました。より網羅的な測定を行うには、ユニットテストフレームワーク（Mocha/Jest）と c8/nyc を組合わせ、すべてのモジュール向けにテストを整備してください。

## 表示系テスト（スクリーンショット）実施状況

- `scripts/screenshot.js` — 存在: はい（リポジトリにコミット済み）
- `cypress/screenshots/` — 存在: いいえ（リポジトリ内にフォルダまたは画像ファイルは存在しません）
- `artifacts/screenshots/` — 存在: いいえ（ローカルで生成したが削除済）
- CI ワークフローの検出（`.github/workflows/ci.yml`）: スクリーンショット添付を必須化するジョブあり（`enforce-screenshots`）

結論:
- 表示系のスクリーンショット自動取得スクリプトは存在するが、現時点で永続的に保存されたスクリーンショットの実ファイルはリポジトリ / ワークスペース内に見つかりません。CI 実行時にアーティファクトとして保存される設計ですが、恒常的な証跡があるかは CI ログ／アーティファクトを確認する必要があります。

